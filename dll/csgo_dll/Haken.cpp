...
void* WINAPI Haken::hookFn(char* hookedFn, char* hookFn, int copyBytesSize, unsigned char* backupBytes, std::string descr)
{
    ...
    {
        std::cout << "[hookFn] Failed to Backup Original Bytes for " << descr << std::endl;
        return nullptr;
    }
    ...
    {
        std::cout << "[hookFn] Failed to set RXW for " << descr << std::endl;
        return nullptr;
    }
    ...
    {
        std::cout << "[hookFn] Failed to Restore Protection for " << std::endl;
    }
...
// Unhook the method
BOOL WINAPI Haken::restore(char* fn, unsigned char* writeBytes, int writeSize, std::string descr)
{
    DWORD oldProtect;
    if (!VirtualProtect(fn, writeSize, PAGE_EXECUTE_READWRITE, &oldProtect))
    {
        std::cout << "[restore] Failed to set RWX for " << descr << std::endl;
        return FALSE;
    }

    if (!WriteProcessMemory(GetCurrentProcess(), fn, writeBytes, writeSize, 0))
    {
        std::cout << "[restore] Failed to Write Memory for " << descr << std::endl;
        return FALSE;
    }

    if (!VirtualProtect(fn, writeSize, oldProtect, &oldProtect))
    {
        std::cout << "[restore] Failed to  Restore Protection for " << descr << std::endl;
        return FALSE;
    }
